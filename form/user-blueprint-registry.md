...
row per type-99 shape: canonical name, meaning, aliases, defining files. It is
curated by hand. This page holds the *why* — allocation rationale and the
registered protocol shapes.

**Current honest split**, witnessed by
...
reviewed bootstrap rows in `form-stdlib/form-ontology-bp.fk` are the current
runtime authority; the registry JSON is the authoring source, and the kernel
does not runtime-parse it; the Go/Rust/TS `bp_table` files are proof-sibling projections,
generated by `form/scripts/bp-table-emit.bml` from the registry, `form-ontology.json`
and the reviewed rows; and the program-image `.fkb` is the target executable
authority, with `.sym` a presentation lens over stable symbols.
