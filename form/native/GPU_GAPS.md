...
- 🟡 **Weight load → device**: **WIRED AND PROVEN ON METAL** for one tensor tile — `form/native/metal/metal_weight_residency_audit.sh` (2026-07-21). REAL llama3.2:3b `blk.0.ffn_down.weight` is located **by name** in the 2 GB blob by Form walking the file's own 7 837 658-byte header (`egg-find-tensor`, equireach-gguf.fk → type 14 Q6_K, dims 8192×3072, abs offset 331 055 328), dequantized by Form (`ewl-weights`), uploaded **once** into an `MTLBuffer`, and dispatched **200×** with **zero re-uploads** (308 µs each, 256×256, Apple M4 Max). Gates: (1) the device buffer is **bit-exact** vs Form's dequantized weights, all 65 536; (2) GPU y is **bit-exact** vs an f32 right-fold over that same resident buffer; (3) vs Form's fp64 `tb-matvec`, `|gpu−form|` is **0.008 of the derived `cols·2⁻²⁴·Σ|term|` fp32 summation bound** (value-relative 8.9e-05, worst-row condition number 6181 — cancellation, not error). The enabling carrier is **equireach** (`form-stdlib/equireach.fk`): before it, this path could not exist at all, because the header had to become a byte list first (PROJECTED ~2.7 h). Still ⬜ **beyond one tile**: whole-tensor residency (25.2M weights ⇒ ~23 min of Form dequant at the measured 18.5k w/s), multi-layer weight sets, KV-cache device buffers, and a `.metallib` cached across runs.
...
- ✅ **Metal (Mac)**: most complete (matvec, affine, mlp, attn, block, llama) — but Mac-only proof (off-Mac the audits SKIP). **+ real-weight residency** (`native/metal/metal_weight_residency_audit.sh`): a named GGUF tensor's Form-dequantized weights live in an `MTLBuffer` and drive the emitted matvec — see §C.
