...
MODS="${MODS/ (read_file \"$S\/core.fk\")/ (read_file \"$S\/core.fk\") (read_file \"$S\/form-ontology-loader.fk\")}"
MODS="${MODS/ (read_file \"$S\/line-grammar.fk\")/ (read_file \"$S\/line-grammar.fk\") (read_file \"$S\/file-byte-window.fk\") (read_file \"$S\/bmf-byte-cursor.fk\") (read_file \"$S\/bmf-core.fk\") (read_file \"$S\/bmf-grammar.fk\") (read_file \"$S\/grammar-loader.fk\") (read_file \"$S\/shell-grammar.fk\")}"
MODS="${MODS/ (read_file \"$S\/storage-port.fk\")/ (read_file \"$S\/storage-port.fk\") (read_file \"$S\/cell-log-store.fk\") (read_file \"$S\/storage-port-file.fk\")}"
MODS="${MODS/ (read_file \"$S\/form-cli-carrier.fk\")/ (read_file \"$S\/form-cli-carrier.fk\") (read_file \"$S\/federation-graph-store.fk\")}"
MODS="${MODS/ (read_file \"$S\/voice-traits.fk\")/ (read_file \"$S\/voice-traits.fk\") (read_file \"$S\/feature-vector.fk\")}"
MODS="${MODS/ (read_file \"$S\/nearest-shape.fk\")/ (read_file \"$S\/nearest-shape.fk\") (read_file \"$S\/voice-diarize.fk\")}"
MODS="${MODS/ (read_file \"$S\/form-cli.fk\")/ (read_file \"$S\/shell-exec.fk\") (read_file \"$S\/form-cli.fk\")}"
BAND="(read_file \"$S/form-cli-repl.fk\")"

# Prefer fkwu self-host flatten (no Go) when T_flat + cached fkwu are warm.
FORM_CLI_SRCS=(
    "$S/fourth-shim.fk" "$S/core.fk" "$S/form-ontology-loader.fk" "$S/offer-ack-core.fk" "$S/federation-graph-offer.fk" "$S/line-grammar.fk"
    "$S/file-byte-window.fk" "$S/bmf-byte-cursor.fk" "$S/bmf-core.fk" "$S/bmf-grammar.fk" "$S/grammar-loader.fk" "$S/shell-grammar.fk"
    ...
    "$S/cell-log-store.fk" "$S/storage-port-file.fk"
    ...
    "$S/voice-traits.fk" "$S/feature-vector.fk"
    "$S/nearest-shape.fk" "$S/voice-diarize.fk" "$S/co-learning.fk" "$S/co-learning-stream.fk"
    ...
    "$S/form-cli-staged-trace.fk" "$S/form-cli-request.fk" "$S/form-cli-carrier.fk" "$S/federation-graph-store.fk" "$S/form-cli-ask-plus.fk"
    "$S/current-branch-landing.fk" "$S/shell-exec.fk"
    "$S/form-cli.fk"
...
SOURCES="$SOURCES form-ontology-loader offer-ack-core federation-graph-offer federation-graph-store cell-log-store storage-port-file file-byte-window bmf-byte-cursor grammar-loader shell-grammar feature-vector voice-diarize shell-exec"
{
