...
| 6 | Witnessed ≥ best rented model | **metric registered; sources seated; claim still pending** | `observe/nvr-metric.fk` **511** + `observe/nvr-corpus.fk` **1023**. Eight held-out sources sit. Complete pairs = 0, so the floor of 8 is not met. A ≥ rented claim is still pending. |
