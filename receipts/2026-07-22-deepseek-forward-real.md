...
incrementally (`a79be5f23`, `2857f1861`), plus corpus row 873 and this receipt.
...
the decode is exact, and to float precision where a matvec reassociates the sum (assocwall, row 872).
...
**token-only** — decided before any layer runs. This is the frontier word `forepick` (row 873).
...
| 1 | token_embd, ffn_gate_inp, ffn_gate_exps, ffn_up_exps, ffn_down_exps each `holds=1` | lapspan (row 871) |
...
from a table before any state that would seem to decide it is computed?* — `forepick`, corpus row 873,
