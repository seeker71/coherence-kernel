...
witness), corpus row 863 `coemit`. IQ2_XXS is 42.6% of the file's bytes and the expert
...
(row 855); it rides in the 8th bit of the transcribed `iq2_ksigns` mask. The kernel does
...
| corpus band | 8191 | **8191** (after row 863, pins re-probed) |
...
**Frontier question, landed as corpus row 863** (`learn/homecoming-distillation-corpus.fk`):
...
the table, not the table (row 855 `paritylock` + the from-spec oracle did that). Distinct from
