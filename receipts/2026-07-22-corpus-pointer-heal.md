...
- a backtick- or quote-wrapped word (`row 847 \`onelean\``) — a whole markdown class, §4
...
can tell you a bare address is wrong. That finding became row 852.
...
- 74 bare-id citations remain unfalsifiable. Row 852 names the property; nothing
  yet requires it.
...
**Row 852 — `apposition`.** *What one word names a reference that states its
...
(row 852). The four `; row NNN (` header heals and the `autopoiesis` fix landed
