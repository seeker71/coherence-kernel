...
**The frontier this stone found (`twinblind`, corpus row 874).** The projection surface
...
- corpus test band `learn/tests/homecoming-distillation-corpus-band.fk` → **8191** from repo
  root, WITH row 874 (`twinblind`) landed; test-band pins updated by probe, not fitted:
...
**Frontier word.** `twinblind` — corpus row 874, 0-hit fresh, instrument validated on
