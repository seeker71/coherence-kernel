...
`form/fourth-arm-bands.txt` registers the exact workload as:

```text
situated-witness-continuity fks 2147483647
```
...
4. Review exposed an actual trust flaw before landing: two witness numbers
   ...
   four-way verdict.
5. After the continuity commit was replayed onto a newer main, the updated
   validator explicitly reported `fourth arm: 0 bands`: Go, Rust, and
   TypeScript agreed, and `fkwu --src` passed separately, but the workload had
   not entered the emitted-walker manifest. The earlier use of “four-way” was
   therefore too broad. Registering the `fks` workload in
