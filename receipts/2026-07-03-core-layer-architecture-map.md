...
| 9h5. Runtime program-image `.fkb` micro-walker | First real trace producer after terminal supplied-evidence 9h4: consume only a request-ready 9e program-image envelope and admitted 8h6 PIF byte admission; walk the admitted in-memory PIF table with bounded fuel for LIT/ARG/CALL/ADD/SUB/MUL/LE/IF, single-evaluation integer EQ/LT and SEQ, packed-list EMPTY/CONS/HEAD/TAIL/LEN/NTH, exact `nothing`/`nothing?`, typed string-pool literal/length/equality/concat/byte-at/substring/find/int-to-string, immutable sparse RESERVE/LETF/FARG frames, strict four-coordinate MAKE_NODEID, native trivial/composite node construction, typed NODE_CHILDREN/NODE_VALUE/NODE_CATEGORY readers, and structural node equality; carry an execution-created parallel kind tree through CALL, CONS, NTH, SEQ, native nodes and occupied frame bindings rather than infer types after execution; bind tag-12 call arguments to image-local function roots without entering the seed's global function table; retain typed present values separately from failed/timeout output absence; emit a computed `rpiwt-trace` plus micro-walk receipt | Supplied run/trace input, bridge/join consumption, delegating to 9h1/9h3/9h4, recipe-walker delegation, artifact IO, table-text bridge, host/native execution, typed public entry envelopes, file/hash/lookup operations, selector/storage/champion/corpus mutation, generated bp-table authority, C-seed growth |
| 9h6. Runtime program-image `.fkb` symbol walk | Symbol-addressed computed runtime surface after 9h5: consume a request-ready 9e program-image envelope, admitted 8h6 PIF byte admission, and 8h8 symbol-id/canonical-key/id+key request; reject diagnostic-node requests; resolve the symbol internally over the admitted PIF; call 9h5 to emit a computed trace with top-level budget/input preservation, including same-process multi-function CALL rows, packed typed values such as `[]`, `nothing`, `0`, `1`, empty/native strings and string `"0"`, plus a strict six-child native NodeID knowledge request with typed readers, present generated zero, and an in-image sparse-frame validator over node/string/integer equality and comparison; retain image-local dependency targets and the full reachable walk-reason surface | Raw entry-index runtime API, supplied symbol-resolution authority, diagnostic-node walking, supplied run/trace input, 9h3/9h4 consumption, 9g capability binding, dependency-metadata authority beyond the admitted call rows, table text or presentation-lens authority, generated proof-sibling table authority, artifact IO, file/hash/lookup operations, selector/storage/champion/corpus mutation, C-seed growth |
...
- `form/form-stdlib/runtime-program-image-fkb-micro-walker.fk` is Layer 9h5,
  ...
  bounded fuel for arithmetic/control, image-local CALL, packed list and exact
  absence tags, plus PIF string-pool literal/length/equality/concat/byte-at/
  substring/find/int-to-string. String pool, type, byte-index and substring
  faults are loud; empty string, string `"0"`, scalar `0`, and present `nothing`
  remain distinct through an execution-created parallel kind tree carried
  across CALL, list construction/selection, and occupied sparse frame bindings,
  without post-hoc reflection. RESERVE establishes only a logical capacity,
  LETF persistently adds one occupied binding, and FARG refuses uninitialized
  or out-of-range reads; a million-slot reserve therefore allocates no seats.
  MAKE_NODEID now requires exactly four integer components at the resident PIF
  boundary; trivial int/string and composite nodes retain their exact subtype
  and child kinds through NODE_CHILDREN, NODE_VALUE, NODE_CATEGORY and sparse
  frames. Structural node equality returns a present integer 0/1. Scalar
  `value_eq` semantics remain outside this strict tag-80 crossing because the
  PIF alias no longer distinguishes it from `node_eq`. Tags 102 and 103 now
  provide exact single-evaluation integer equality and less-than; SEQ observes
  its left child and preserves the right child's exact value kind.
  The public entry still admits a raw integer input rather than a typed entry
  envelope. Budget exhaustion becomes a timeout trace; unsupported tags
  and invalid child references become loud error traces. It does not accept supplied runs, supplied traces, bridges,
...
- `form/form-stdlib/runtime-program-image-fkb-symbol-walk.fk` is Layer 9h6,
  ...
  value plus the 8h8 resolution, image-local dependency targets, 9h5 micro-walk
  receipt, and computed trace so typed string/native-node values and every
  reachable refusal/timeout/error reason stay observable. The canonical
  `knowledge/request` symbol constructs category `31.2.0.98` with six native
  children (`concept`, `choice`, `96`, `1`, present `0`, and the scannerless
  BMF cursor protocol) in 30 steps; budget 29 returns timeout with no output.
  A sparse-frame validator stores that request once and validates all fields,
  including exact payload 96, in 111 steps; the unchanged validator rejects a
  six-child payload-95 request as present zero in 84 steps, while budget 110 times out
  with no output. A SEQ inside the same retained frame completes in 114 steps
  and times out at 113 without rebuilding the request or collapsing either
  state. It
