...
witness, **VERDICT PASS, 10 gates**). Plus corpus row 862 `sumkeel`.
...
**Frontier question, landed.** Row 862 `sumkeel` — one word for an output that must obey a
...
hit), and landed as a real `(hdc-row 862 …)`. The corpus band crosses at 8191 with it
