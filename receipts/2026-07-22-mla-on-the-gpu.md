...
GPU-fidelity. Cites: row 865 `hushfold` (RoPE is the identity at position 0, so the ≥2-position test is
what witnessed it), row 849 `asktoll` (the per-head slot), `receipts/2026-07-22-mla-recipe.md` (the CPU
recipe this equals).
