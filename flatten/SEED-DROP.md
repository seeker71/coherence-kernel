# SEED-DROP - a flatten/`form-eval` door (no longer a gate; the body already runs via the source door)
...
> C **table-executor**, NOT the the source door parser (`fk_sparse`): `fkwu flatten/form-eval-cli-loop.tbl 0 t.fk`
> with `(add 40 2)` → 42, `(do (let x 40) (add x 2))` → 42 (the same `let` via the source door returns 2 — a
...
> `receipts/2026-06-29-src-stone5-real-cell-on-fkwu.md`. So "standing the body on Windows" is **done** via the source door.
>
...
The kernel runs the Form body as Form today via the source door. The seed below is the optional `form-eval` door - a
...
the source door surface, name the actual source/lowering coverage gap; do not call it a missing table seed.
