...
- [x] **Speech neural pair coverage added.** `learn/speech-neural-pair-coverage.fk` makes the pair-training
      boundary executable (`32767`): locale `A=>neural=>B` coverage is now `10/55` broad ready pairs and `20/110`
      directed routes, with `10` neural epochs and `10` native neural parameters. The separate Form-native seeded
...
- [x] **Speech pair training next action added.** `learn/speech-pair-training-next-action.fk` now chooses the next
      executable neural movement (`32767`): choose `next-native-neural-pair-window-0011` over `sa<->la`, route
      `train-native-neural-pair-window`, keep Form-native pair windows `8 -> 8`, move neural pair windows `10 -> 11`,
