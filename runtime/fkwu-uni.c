...
static long long fk_walk_cold(long long t, long long i, long long fp) {
    ...
    if (t == 29) {
        long long sa = fk_walk(fk_node[i][1], fp) >> 1;
        long long a = fk_walk(fk_node[i][2], fp) >> 1;
        long long b = fk_walk(fk_node[i][3], fp) >> 1;
        if (sa < 0 || sa >= fk_sp || a < 0 || b < a || b > fk_sl[sa]) {
            return 0 - 2;
        }
        long long ln = b - a;
        while (fk_sbp + ln > fk_scap_b) {
        ...
        long long j2 = 0;
        while (j2 < ln) {
            fk_sb[fk_sbp + j2] = fk_sb[fk_so[sa] + a + j2];
            j2 = j2 + 1;
        }
        return fk_sintern(fk_sbp, ln) << 1;
    }
    ...
    if (t == 32) {
        char tmp[64];
        long long vr = fk_walk(fk_node[i][1], fp);
        long long n = 0;
        if (vr == (0 - 9223372036854775807LL)) {
            tmp[0] = 116;
            tmp[1] = 114;
            tmp[2] = 117;
            tmp[3] = 101;
            n = 4;
        } else if (vr == (0 - 9223372036854775805LL)) {
            tmp[0] = 102;
            tmp[1] = 97;
            tmp[2] = 108;
            tmp[3] = 115;
