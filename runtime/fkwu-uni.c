...
struct timeval { long tv_sec; int tv_usec; }; extern int gettimeofday(struct timeval *, void *); static long long fk_now_ms(void) { struct timeval tv; if (gettimeofday(&tv, 0) != 0) { return 0; } return ((long long)tv.tv_sec * 1000LL) + ((long long)tv.tv_usec / 1000LL); } static long long fk_elapsed_ms(long long start) { long long end = fk_now_ms(); if (start <= 0 || end <= start) { return 1; } return end - start; }
 ...
 if (t == 249) { static char ffp[2048]; static char ffn[512]; fk_cstr(fk_walk(fk_node[i][1], fp), ffp, 2048); fk_cstr(fk_walk(fk_node[i][2], fp), ffn, 512); void *fh = (ffp[0] == 0) ? dlopen((const char *)0, 2) : dlopen(ffp, 2); long long fr = (fh == 0) ? 0 : (long long)dlsym(fh, ffn); return fr << 1; }
 if (t == 250) { long long cfp = fk_walk(fk_node[i][1], fp) >> 1; long long lst = fk_walk(fk_node[i][2], fp); long long ca[11]; int cz = 0; while (cz < 11) { ca[cz] = 0; cz = cz + 1; } cz = 0; while ((lst & 1) && lst != 1 && cz < 11) { long long idx = lst >> 1; ca[cz] = fk_hh[idx] >> 1; lst = fk_ht[idx]; cz = cz + 1; } long long (*cfn)(long long, long long, long long, long long, long long, long long, long long, long long, long long, long long, long long) = (long long (*)(long long, long long, long long, long long, long long, long long, long long, long long, long long, long long, long long))cfp; long long cr = (cfn == 0) ? 0 : cfn(ca[0], ca[1], ca[2], ca[3], ca[4], ca[5], ca[6], ca[7], ca[8], ca[9], ca[10]); return cr << 1; }
 if (t == 242) { long long cn = fk_walk(fk_node[i][1], fp) >> 1; void *m = calloc((unsigned long)(cn > 0 ? cn : 1), 1); return ((long long)m) << 1; }
 if (t == 251) { long long cd = fk_walk(fk_node[i][1], fp) >> 1; long long cs = fk_walk(fk_node[i][2], fp) >> 1; long long cnn = fk_walk(fk_node[i][3], fp) >> 1; long long j3 = 0; while (j3 < cnn) { ((char *)cd)[j3] = ((char *)cs)[j3]; j3 = j3 + 1; } return 0; }
 if (t == 233) { static char frp[4096]; fk_cstr(fk_walk(fk_node[i][1], fp), frp, 4096); long long fd_d = fk_walk(fk_node[i][2], fp) >> 1; long long fd_n = fk_walk(fk_node[i][3], fp) >> 1; int fd = open(frp, 0); if (fd < 0) { return -1; } long long got = 0; while (got < fd_n) { long long r = read(fd, (char *)fd_d + got, (unsigned long)(fd_n - got)); if (r <= 0) { break; } got = got + r; } close(fd); return got << 1; }
 if (t == 234) { static char fwp[4096]; fk_cstr(fk_walk(fk_node[i][1], fp), fwp, 4096); long long fw_s = fk_walk(fk_node[i][2], fp) >> 1; long long fw_n = fk_walk(fk_node[i][3], fp) >> 1; int fd = open(fwp, 1 | 0100 | 01000, 0666); if (fd < 0) { return -1; } long long put = 0; /* O_WRONLY|O_CREAT|O_TRUNC, Linux/Android values (this carrier writes on the device) */ while (put < fw_n) { long long w = write((long long)fd, (char *)fw_s + put, (unsigned long)(fw_n - put)); if (w <= 0) { break; } put = put + w; } close(fd); return put << 1; }
 if (t == 252) { return fk_f32op(0, (unsigned int)(fk_walk(fk_node[i][1], fp) >> 1), (unsigned int)(fk_walk(fk_node[i][2], fp) >> 1)) << 1; }
 if (t == 253) { return fk_f32op(1, (unsigned int)(fk_walk(fk_node[i][1], fp) >> 1), (unsigned int)(fk_walk(fk_node[i][2], fp) >> 1)) << 1; }
 if (t == 254) { return fk_f32op(2, (unsigned int)(fk_walk(fk_node[i][1], fp) >> 1), (unsigned int)(fk_walk(fk_node[i][2], fp) >> 1)) << 1; }
 if (t == 255) { return fk_f32op(3, (unsigned int)(fk_walk(fk_node[i][1], fp) >> 1), (unsigned int)(fk_walk(fk_node[i][2], fp) >> 1)) << 1; }
 if (t == 232) { return fk_f32op(4, (unsigned int)(fk_walk(fk_node[i][1], fp) >> 1), 0) << 1; }
 if (t == 243) { long long cp = fk_walk(fk_node[i][1], fp) >> 1; if (cp) { free((void *)cp); } return 0; }
 if (t == 244) { long long cp = fk_walk(fk_node[i][1], fp) >> 1; long long co = fk_walk(fk_node[i][2], fp) >> 1; return ((long long)(*(unsigned int *)(cp + co))) << 1; }
 if (t == 245) { long long cp = fk_walk(fk_node[i][1], fp) >> 1; long long co = fk_walk(fk_node[i][2], fp) >> 1; long long cv = fk_walk(fk_node[i][3], fp) >> 1; *(unsigned int *)(cp + co) = (unsigned int)cv; return 0; }
 if (t == 246) { long long cp = fk_walk(fk_node[i][1], fp) >> 1; long long co = fk_walk(fk_node[i][2], fp) >> 1; return ((long long)(*(unsigned long long *)(cp + co))) << 1; }
 if (t == 247) { long long cp = fk_walk(fk_node[i][1], fp) >> 1; long long co = fk_walk(fk_node[i][2], fp) >> 1; long long cv = fk_walk(fk_node[i][3], fp) >> 1; *(unsigned long long *)(cp + co) = (unsigned long long)cv; return 0; }
 if (t == 248) { const char *sp2; long long sl2; fk_srange(fk_walk(fk_node[i][1], fp), &sp2, &sl2); char *m = malloc((unsigned long)sl2 + 1); long long j2 = 0; while (j2 < sl2) { m[j2] = sp2[j2]; j2 = j2 + 1; } m[sl2] = 0; return ((long long)m) << 1; }
#ifndef _WIN32
...
/* stone 4: a function table. Each top-level (defn name ...) gets its own fn-index (>=1); calls to
   registered names lower to tag 12 for arity 0/1 and tag 240 for multi-arg. A non-defn top form is the root (fn[0]). */
...
static long long fk_parse_call_args(long long n);
static long long fk_sparse(void) {
 ...
 if (c == 40) {
  ...
  /* call: any registered function name (INCLUDING self — each defn is registered before its body is
     parsed) -> tag 12 for arity 0/1, tag 240 for multi-arg. The multi-arg path stores an internal
     parse-list of unevaluated operand nodes so the evaluator can push every callee frame slot. */
  long long fidx = fk_fn_lookup(s, hn);
  if (fidx >= 0) {
   long long ar = (fidx >= 0 && fidx < 4096) ? fk_fnar[fidx] : 1;
   if (ar >= 2) {
    long long args = fk_parse_call_args(ar);
    fk_sskip(); if (fk_spos < fk_slen && fk_srctext[fk_spos] == 41) { fk_spos = fk_spos + 1; }
    return fk_smknode(240, fidx, args, ar);
   }
   long long c1 = (ar == 1) ? fk_sparse() : fk_smklit(0);
   fk_sskip(); if (fk_spos < fk_slen && fk_srctext[fk_spos] == 41) { fk_spos = fk_spos + 1; }
...
static long long fk_parse_call_args(long long n) {
 if (n <= 0) { return -1; }
 long long h = fk_sparse();
 long long t = fk_parse_call_args(n - 1);
 return fk_smknode(241, h, t, 0);
}
...
/* one top-level form: (do ...) is transparent (its inner forms are top-level too); (defn ...) registers
   ...
   to slots 0..k-1, and multi-arg calls populate the same slot frame through tag 240. */
