...
static long long fk_sparse(void) {
 ...
 if (c == 40) {
  ...
  if (fidx >= 0) {
   ...
   if (ar >= 2) {
    long long c1 = fk_sparse(); long long c2 = fk_sparse();
    fk_sskip(); if (fk_spos < fk_slen && fk_srctext[fk_spos] == 41) { fk_spos = fk_spos + 1; }
    return fk_smknode(240, fidx, c1, c2);
   }
