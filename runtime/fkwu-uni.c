...
static long long fk_sparse(void) {
 ...
 if (c == 40) {
  ...
  /* unknown head (no op / rewrite / user fn matched): consume the WHOLE balanced
     form so the parser stays aligned for what follows. The old skip stopped at the
     first ')', so a nested unknown like (append (w s) (b s)) left the tail as garbage
     and silently corrupted every later defn. Balance the parens (respecting "..."
     string literals) and return an honest 0 for just this node. */
