...
/* ── stone 3 (OBSERVE): the offer/ack observe hook ──────────────────────────
   Every reducer CALL is an OFFER (axiom-5): a callee + its args, acknowledged by
   EXACTLY ONE of {nothing, 0, 1, node}. This hook makes that offer/ack witnessable
   as a trace the observe organ reads — the live feed runtime-witness.fk named as
   the one piece "that depends on the runtime emitting it" (a fire-event per call).
   It composes the existing fk_arms tag-counter (which already witnesses every node
   visit) by LIFTING it to the offer/ack altitude: one line per offer, carrying the
   callee, the arg-count, and the FOUR-ARM ack-kind the call returned.
   Toggle: env FK_OBSERVE=1 (read once). OFF -> fk_observe is 0, the branch is a
   single predicted-false test per call and emits NOTHING: zero output, no alloc,
   no formatting. The mind watches itself think only when asked to. */
static long long fk_observe = -1;   /* -1 = unread; 0 = off; 1 = on */
static long long fk_observe_on(void) { if (fk_observe < 0) { char *e = getenv("FK_OBSERVE"); fk_observe = (e && e[0] && e[0] != 48) ? 1 : 0; } return fk_observe; }
/* the FOUR-ARM ack classifier (axiom-1 order: nothing first, the ground). A raw
   reducer ack is exactly one of: nothing (the canonical sentinel) | 0 (the zero
   state) | node (a content-addressed cell back, a counter-offer) | 1 (every other
   affirmative result — the carried payload). This is the ONE place the four arms
   are read off a raw value; everything that needs the kind reads through it (no
   per-call-site ack if-chain). */
static long long fk_ack_kind(long long v) {
 if (fk_is_nothing(v)) { return 0; }                 /* nothing — the silence/decline ack */
 if (v < 0) { long long ni = fk_nidx(v); if (ni >= 1 && ni <= fk_np) { return 3; } }   /* node — counter-offer */
 if (v == 0) { return 1; }                           /* 0 — the zero state */
 return 2;                                            /* 1 — the affirmative result (any payload) */
}
/* emit one offer/ack trace line (only when observing). Shape the observe organ
   reads: a fire-event lifted to offer/ack — `offer <callee> args=<n> ack=<arm>`.
   callee is the function index offered; <n> the args packed; <arm> the four-arm
   name. Returns v unchanged so it wraps a call's return transparently. */
...
static long long fk_sparse(void) {
 ...
 if (c == 40) {
  ...
  if (fidx >= 0) {
   /* GENERAL ARITY (no per-arity case): parse the callee's `ar` declared arg
      expressions and thread them into a forward-linked arg-chain of tag-242
      cells (cell: [1]=arg-expr-node, [2]=next-cell or -1, head-first so slot 0
      is the first arg). The call is ONE tag-241 node ([1]=fidx, [2]=chain-head
      or -1). fk_walk evaluates the chain left-to-right, pushing each arg via
      fk_vp exactly as the table path packs N args — same mechanism, any N.
      ar==0 parses no args (chain -1, inert); ar==1/2/8 are the same code. */
   long long ar = (fidx >= 0 && fidx < 4096) ? fk_fnar[fidx] : 1;
   long long argn[256]; long long ai = 0;
   while (ai < ar && ai < 256) { argn[ai] = fk_sparse(); ai = ai + 1; }
   fk_sskip(); if (fk_spos < fk_slen && fk_srctext[fk_spos] == 41) { fk_spos = fk_spos + 1; }
   long long chain = -1; long long k = ai;
   while (k > 0) { k = k - 1; chain = fk_smknode(242, argn[k], chain, 0); }
   return fk_smknode(241, fidx, chain, 0);
  }
  /* INDIRECT CALL (stone 2c): a call (h args..) whose head h is a BOUND NAME — a
     parameter, or a let-var holding a fn returned from a fn — is an offer to a
     COMPUTED callee (axiom-5: offer a computed cell). The head LOWERS to its slot
     read (tag 110); at eval it must reduce to a fn-VALUE, and the fn it names is
     then offered with the args. Emit tag 244: [1]=head-expr-node, [2]=arg-chain
     (242 cells, head-first), exactly the tag-241 shape but with a computed head.
     Args are parsed until the close paren (the indirect callee's arity is not a
     static name lookup); each is a forward-linked 242 cell so fk_walk threads them
     left-to-right like the direct path. A bare fn-NAME never reaches here (it
     resolves at fk_fn_lookup above into the direct tag-241 path). */
  long long hoff = fk_bd_lookup(s, hn);
  if (hoff >= 0) {
   long long head244 = fk_smknode(110, fk_smklit(hoff), 0, 0);
   long long iargn[256]; long long iai = 0;
   while (iai < 256) { fk_sskip(); if (fk_spos >= fk_slen || fk_srctext[fk_spos] == 41) { break; } iargn[iai] = fk_sparse(); iai = iai + 1; }
   fk_sskip(); if (fk_spos < fk_slen && fk_srctext[fk_spos] == 41) { fk_spos = fk_spos + 1; }
   long long ichain = -1; long long ik = iai;
   while (ik > 0) { ik = ik - 1; ichain = fk_smknode(242, iargn[ik], ichain, 0); }
   return fk_smknode(244, head244, ichain, 0);
  }
  /* unknown head (no op / rewrite / user fn matched): an OFFER to a callee that
     does not resolve. stone 3 (2b-ii): axiom-5 — "an offer a cell can't answer
     acks nothing." So a (head ...) whose head is NOT a resolvable callee yields
     the canonical first-class nothing (tag 137), NOT a literal 0. This is the
     INTENTIONAL nothing: the offer-shape is well-formed (a balanced (head args..))
     but no cell answers it — exactly fail/decline. It is what oac-choice (first
     non-nothing) and oac-try (recover from nothing) recover over REAL reducer
     calls, not only explicit (nothing).
       The line vs a MASKED BUG: we only reach here from a CALL position (inside
     `(`, after the head matched no op/rewrite/fn) — a well-formed offer to a
     non-resolving callee. We still consume the WHOLE balanced form so the parser
     stays aligned (the old first-`)` skip corrupted later defns); a genuinely
     malformed program does not become silent here — unbalanced parens still run
     the source past its end, and a structurally broken op (wrong arity, bad
     literal) surfaces through its own op path, never through this call-decline.
     A bare unbound SYMBOL (a value position, not a call/offer) stays 0 below —
     it is not an offer, so it does not ack nothing. */
  { long long depth = 1; while (fk_spos < fk_slen && depth > 0) { char cc = fk_srctext[fk_spos]; if (cc == 34) { fk_spos = fk_spos + 1; while (fk_spos < fk_slen && fk_srctext[fk_spos] != 34) { fk_spos = fk_spos + 1; } if (fk_spos < fk_slen) { fk_spos = fk_spos + 1; } continue; } if (cc == 40) { depth = depth + 1; } else if (cc == 41) { depth = depth - 1; } fk_spos = fk_spos + 1; } }
 ...
 /* a bare symbol: a bound name -> tag 110 (read its frame slot); a registered fn-NAME
    in VALUE position (stone 2c) -> tag 243, the fn-VALUE (an odd-negative reserved
    sentinel naming the fn-index — first-class, passable as an arg, storable, returnable);
    else an honest 0. A bound name wins over a fn-name (lexical scope shadows). */
...
static void fk_parse_top(void) {
 ...
 if (fk_srctext[fk_spos] == 40) {
  ...
  if (fk_sym_eq(p, he - p, "defn")) {
   ...
   if (idx >= 0 && idx < 4096) { fk_fnar[idx] = na; } /* arity known before body -> self-recursive calls read it */
   long long body = fk_sparse();
   fk_sskip(); if (fk_spos < fk_slen && fk_srctext[fk_spos] == 41) { fk_spos = fk_spos + 1; }
   if (fk_maxslot > 0) { body = fk_smknode(111, fk_smklit(fk_maxslot), body, 0); }
...
/* ── GENERAL primitive + inter-fn CARRIERS (the JIT calls these, not per-op asm) ──
   The reframe: "string/float/list ops" are NOT special JIT features — they are RECIPES
   over primitives. The JIT only needs (1) any-kind literals (handled by emitting the
   interned tagged word) and (2) the ability to emit a CALL to a kind-correct carrier.
   Then ALL recipes lower as recipes; the value-correctness lives in ONE place (the same
   computation fk_walk runs), so a lowered float add can NEVER drift from the walker.

   fk_jprim2(tag,a,b) / fk_jprim1(tag,a) take ALREADY-EVALUATED tagged words and return
   the tagged result — bit-identical to fk_walk's tag case. CORRECTNESS NOTE: add/sub/mul/
   le/eq are float-aware here (via fk_num/fk_fbox), so lowering them as carrier calls is
   correct for float operands (the #59 int-inline was WRONG for floats — fchk would return
...
static long long fk_jprim2(long long tag, long long a, long long b) {
 if (tag == 3) { if (fk_isf(a) || fk_isf(b)) { return fk_fbox(fk_num(a) + fk_num(b)); } return a + b; }
 if (tag == 4) { if (fk_isf(a) || fk_isf(b)) { return fk_fbox(fk_num(a) - fk_num(b)); } return a - b; }
 ...
 if (tag == 27) {   /* str_concat — mirrors fk_walk's tag-27 exactly */
  ...
  while (fk_sbp + ln > fk_scap_b) { fk_scap_b = fk_scap_b * 2; fk_sb = realloc(fk_sb, fk_scap_b); }
  long long off = fk_sbp; long long k = 0;
  while (k < fk_sl[sa]) { fk_sb[off + k] = fk_sb[fk_so[sa] + k]; k = k + 1; }
  long long m = 0; while (m < fk_sl[sb]) { fk_sb[off + fk_sl[sa] + m] = fk_sb[fk_so[sb] + m]; m = m + 1; }
  return fk_sintern(off, ln) << 1;
 }
 if (tag == 26) { return fk_keyeq(a >> 1, b >> 1) ? 2 : 0; }   /* str_eq — mirrors fk_walk tag-26 */
 if (tag == 28) {   /* str_byte_at — mirrors fk_walk tag-28 */
  long long sa = a >> 1; long long k = b >> 1;
  if (sa < 0 || sa >= fk_sp || k < 0 || k >= fk_sl[sa]) { return 0 - 2; }
...
static long long fk_jprim3(long long tag, long long a, long long b, long long c) {
 if (tag == 29) {   /* substring — mirrors fk_walk tag-29 */
  long long sa = a >> 1; long long lo = b >> 1; long long hi = c >> 1;
  if (sa < 0 || sa >= fk_sp || lo < 0 || hi < lo || hi > fk_sl[sa]) { return 0 - 2; }
  long long ln = hi - lo;
  while (fk_sbp + ln > fk_scap_b) { fk_scap_b = fk_scap_b * 2; fk_sb = realloc(fk_sb, fk_scap_b); }
  long long j = 0; while (j < ln) { fk_sb[fk_sbp + j] = fk_sb[fk_so[sa] + lo + j]; j = j + 1; }
  return fk_sintern(fk_sbp, ln) << 1;
 }
 return fk_nothing;
}
static long long fk_jprim1(long long tag, long long a) {
 if (tag == 25) { long long sa = a >> 1; if (sa < 0 || sa >= fk_sp) { return 0; } return fk_sl[sa] << 1; }  /* str_len */
 if (tag == 54) { return ((long long)fk_num(a)) << 1; }                                                    /* float_to_int */
 if (tag == 53) {   /* str_to_float — mirrors fk_walk's tag-53 exactly */
  long long sa = a >> 1; if (sa < 0 || sa >= fk_sp) { return fk_fbox(0.0); }
  char tmp[128]; long long n = fk_sl[sa]; if (n > 126) { n = 126; }
  long long j = 0; while (j < n) { tmp[j] = fk_sb[fk_so[sa] + j]; j = j + 1; } tmp[n] = 0;
...
/* ── ensure `callee` has an installed native; return its frame-slot count via *frame,
   or 0 if it cannot crystallize. Heat-gated and tried-once so a non-lowering callee is
   marked and never re-lowered (falls through to the walker). Shared by fk_jcall (the
   --src inter-fn site) and fk_feval_try_native (the --feval trampoline) — one lower+install
   path, one cache (fk_src_nat / fk_nat_exec / fk_nat_tried / fk_fheat). Lowering also sets
   fk_jit_frame as a side-effect; we capture and stash it per-callee in fk_src_nat_frame so
   the direct-dispatch caller knows the frame size without re-lowering. */
static long long fk_src_nat_frame[4096];
/* eager=1 (the --src inter-fn site): being CALLED from JITed code is itself the heat signal —
   crystallize on first request so a mutual-recursion pair flips native↔native immediately and
   does not stay in the walker after one deopt. eager=0 (the --feval trampoline): heat-gate as
   before so cold fns don't pay lowering cost. */
static fk_natfn fk_ensure_native_ex(long long callee, long long *frame, int eager) {
 if (callee < 0 || callee >= 4096) { return 0; }
 if (fk_nat_exec[callee] != 0) { if (frame) { *frame = fk_src_nat_frame[callee]; } return fk_nat_exec[callee]; }
 if (fk_src_nat[callee] == 0) {
  if (fk_nat_tried[callee]) { return 0; }
  if (!eager) {
   fk_fheat[callee] = fk_fheat[callee] + 1;
   if (fk_fheat[callee] < fk_feval_hot) { return 0; }
  }
  fk_nat_tried[callee] = 1;
  long long n = fk_jit_lower(callee);
  if (n <= 0) { return 0; }
  long long fr = fk_jit_frame;
  unsigned char *img = malloc(n); if (img == 0) { return 0; }
  long long ci = 0; while (ci < n) { img[ci] = fk_jb[ci]; ci = ci + 1; }
  fk_src_nat[callee] = img; fk_src_nat_len[callee] = n; fk_src_nat_frame[callee] = fr; fk_njit = fk_njit + 1;
  if (getenv("FK_JIT_WITNESS")) { printf("[jit] fn%lld crystallized: %lld bytes, njit=%lld (direct dispatch ready)\n", callee, n, fk_njit); }
 }
 if (fk_nat_exec[callee] == 0) { fk_nat_exec[callee] = fk_nat_install(fk_src_nat[callee], fk_src_nat_len[callee]); if (fk_nat_exec[callee] == 0) { return 0; } }
 if (frame) { *frame = fk_src_nat_frame[callee]; }
 return fk_nat_exec[callee];
}
static fk_natfn fk_ensure_native(long long callee, long long *frame) { return fk_ensure_native_ex(callee, frame, 0); }
/* inter-fn CALL carrier: dispatch fn `callee` with `argc` args. The lowered call to another
   fn routes here, so a JITed fn can call ANY other fn correctly.

   DIRECT NATIVE→NATIVE dispatch (the speed lever): if the callee already has an installed
   native AND the call arity matches the arity that native was lowered for, set up the rooted
   args frame at fk_vs[fp..] and JUMP STRAIGHT to the native entry — no fk_walk_body bounce.
   The args (live tagged values, possibly cons) are spilled into fk_vs[fp..fp+argc) and fk_vsp
   is raised over the WHOLE frame (args + let-locals), so a compacting fk_melt triggered inside
   the callee scans these slots as roots and RELOCATES the cons pointers the native stores there
   (GC-correct, exactly as fk_feval_try_native does). On arity mismatch or no-native → fall
   through to fk_walk_body (the walker stays source of truth; deopt is always safe). */
/* ── NATIVE TAIL-CALL TRAMPOLINE ──────────────────────────────────────────────────────────
   A direct native→native CALL recurses on the C machine stack: fine for TREE recursion (fib
   depth = tree height, small) and bounded LINEAR recursion, but a deep TAIL chain (ev/od 20M
   deep) would blow the 256 MB stack. The walker solves this with a trampoline (constant stack);
   the native path needs its own. The lowerer emits a TAIL-position inter-fn call NOT as a call
   but as: write the new args into the current rbp frame IN PLACE, record the next callee in
   fk_tail_callee, and RETURN the fk_tailcall sentinel. The C driver fk_jtramp loops on that
   sentinel — dispatching the next native over the SAME fp frame — so a mutual-recursion chain
   runs native↔native end-to-end in CONSTANT stack, no walker bounce.

   fk_tailcall is a reserved ODD-NEGATIVE sentinel in a band no tagged value occupies: ints are
   even (v<<1); floats are <= fk_fbase-2 (~-9e18); nodes/records are small-magnitude negatives;
   cons are positive-odd; fnvals sit in the -8e18±16384 band; nothing is -8.999e18. -7.5e18-1 is
   odd, far above the float floor, far below node magnitudes, and outside the fnval band — so it
...
/* gate: emit the tail-trampoline sentinel form for tail inter-fn calls. ON for the --src JIT
   path (where native↔native tail-chaining is proven correct + bounded under melt). OFF for the
   --feval interpreter path, which keeps the #75 wire (tail inter-fn calls lower as non-tail
   fk_jcall → walker bounce): correct + bounded for the meta-evaluator's heavy env-consing,
   which the native chain does not yet root register-resident cons across. (Honest scope: the
   register-spill rung named in the #75 receipt is what would let --feval chain natively too.) */
...
static void fk_jtail_set(long long callee) { fk_tail_callee = callee; }
/* drive a native call at frame `fp` (args already in fk_vs[fp..fp+argc)); loop while the native
   returns the tail sentinel, dispatching the recorded next callee over the SAME frame. argc is
   the fixed chain arity; a tail target whose declared arity differs, or that can't crystallize,
   deopts to the walker (which itself trampolines) and ends the native chain — always correct. */
static long long fk_jtramp(long long callee, long long fp, long long argc) {
 long long save_vsp = fk_vsp;
 for (;;) {
  if (callee < 0 || callee >= 4096 || fk_fnar[callee] != argc) {
   long long r = fk_walk_body(fk_fn[callee], fp); fk_vsp = save_vsp; return r;
  }
  long long frame = 0;
  fk_natfn nf = fk_ensure_native_ex(callee, &frame, 1);   /* eager: a call FROM jit IS the heat */
  if (nf == 0) { long long r = fk_walk_body(fk_fn[callee], fp); fk_vsp = save_vsp; return r; }
  long long fr = frame; if (fr < argc) { fr = argc; }
  /* HEADROOM: this native may end in a TAIL inter-fn call that writes up-to-6 args into
     ...
     rooted region. Reserve at least the max lowered arity so every in-place tail rewrite stays
     inside the melt-scanned, non-overlapping frame. (Generous reserve is always safe: sub-calls
     take fp2=fk_vsp above it; melt scans it; cost is a few vsp slots per chain.) */
  if (fr < 6) { fr = 6; }
  long long z = argc; while (z < fr) { if (fp + z < 65536) { fk_vs[fp + z] = 0; } z = z + 1; }
  long long need = fp + fr; if (need > fk_vsp && need < 65536) { fk_vsp = need; }
  fk_tail_callee = -1;
  long long r = nf(&fk_vs[fp]);
  if (r == fk_tailcall) { callee = fk_tail_callee; continue; }  /* args already rewritten into fk_vs[fp..] */
  fk_vsp = save_vsp;
  return r;
 }
}
/* inter-fn CALL carrier (NON-tail call site). Runs its own nested trampoline so a tail-call
   sentinel raised inside the callee's chain is absorbed here and never escapes past this frame.
   The args (live tagged values, possibly cons) are spilled into fk_vs[fp..fp+argc); fk_jtramp
   raises fk_vsp over the whole frame so a compacting fk_melt scans these slots as roots and
   RELOCATES the cons pointers the native stores there (GC-correct, as fk_feval_try_native). */
static long long fk_jcall(long long callee, long long argc, const long long *args) {
 ...
 long long r = fk_lower_tail_tramp ? fk_jtramp(callee, fp, argc) : fk_walk_body(fk_fn[callee], fp);
 fk_vsp = fp;
 return r;
}
...
/* ── emit a CALL to a C carrier at absolute address `fn`, with `nargs` (0..3) args
   already pushed on the machine stack (last-pushed = argN-1, so they pop in order).
   Robust stack realignment (works at any incoming alignment): save rsp, align to 16,
   pad + shadow, pop the staged args into the ABI arg registers, call, restore rsp.
   Result (tagged word) is left in rax. Win64 ABI: args in rcx,rdx,r8 (+32B shadow);
   SysV: rdi,rsi,rdx (no shadow). The carrier preserves rbp (callee-saved), so the
   args pointer survives the call. */
static void fk_jcarrier(void *fn, int nargs) {
 /* operands are on the stack: caller pushed arg_{n-1} first ... arg0 last, so
    arg0 = [rsp+0], arg1 = [rsp+8], ...  Read them into the ABI arg registers, then
    realign + call, then restore rsp to ABOVE the consumed args (rsp + nargs*8). */
#if defined(_WIN32)
 if (nargs >= 1) { fk_jb1(0x48); fk_jb1(0x8B); fk_jb1(0x0C); fk_jb1(0x24); }            /* mov rcx,[rsp] */
 if (nargs >= 2) { fk_jb1(0x48); fk_jb1(0x8B); fk_jb1(0x54); fk_jb1(0x24); fk_jb1(8); } /* mov rdx,[rsp+8] */
 if (nargs >= 3) { fk_jb1(0x4C); fk_jb1(0x8B); fk_jb1(0x44); fk_jb1(0x24); fk_jb1(16);} /* mov r8,[rsp+16] */
 if (nargs >= 4) { fk_jb1(0x4C); fk_jb1(0x8B); fk_jb1(0x4C); fk_jb1(0x24); fk_jb1(24);} /* mov r9,[rsp+24] */
#else
 if (nargs >= 1) { fk_jb1(0x48); fk_jb1(0x8B); fk_jb1(0x3C); fk_jb1(0x24); }            /* mov rdi,[rsp] */
 if (nargs >= 2) { fk_jb1(0x48); fk_jb1(0x8B); fk_jb1(0x74); fk_jb1(0x24); fk_jb1(8); } /* mov rsi,[rsp+8] */
 if (nargs >= 3) { fk_jb1(0x48); fk_jb1(0x8B); fk_jb1(0x54); fk_jb1(0x24); fk_jb1(16);} /* mov rdx,[rsp+16] */
 if (nargs >= 4) { fk_jb1(0x48); fk_jb1(0x8B); fk_jb1(0x4C); fk_jb1(0x24); fk_jb1(24);} /* mov rcx,[rsp+24] */
#endif
 /* compute the post-consume rsp into r11 (caller-saved, but we use it only here): */
 fk_jb1(0x4C); fk_jb1(0x8D); fk_jb1(0x5C); fk_jb1(0x24); fk_jb1((unsigned char)(nargs * 8)); /* lea r11,[rsp+nargs*8] */
 fk_jb1(0x48); fk_jb1(0x83); fk_jb1(0xE4); fk_jb1(0xF0); /* and rsp,-16 (align down) */
 fk_jb1(0x41); fk_jb1(0x53);                          /* push r11  (save restore-target) */
 fk_jb1(0x41); fk_jb1(0x53);                          /* push r11  (pad -> rsp 16-aligned) */
#if defined(_WIN32)
 fk_jb1(0x48); fk_jb1(0x83); fk_jb1(0xEC); fk_jb1(32);/* sub rsp,32 (Win64 shadow; keeps 16-align) */
#endif
 fk_jb1(0x48); fk_jb1(0xB8); fk_jb8((long long)(unsigned long long)fn); /* mov rax,&fn */
 fk_jb1(0xFF); fk_jb1(0xD0);                          /* call rax  (result -> rax) */
#if defined(_WIN32)
 fk_jb1(0x48); fk_jb1(0x83); fk_jb1(0xC4); fk_jb1(32);/* add rsp,32 */
#endif
 fk_jb1(0x48); fk_jb1(0x83); fk_jb1(0xC4); fk_jb1(8); /* add rsp,8 (discard pad) */
 fk_jb1(0x5C);                                        /* pop rsp  (rsp <- saved restore-target) */
}
static long long fk_jprim2(long long tag, long long a, long long b);
static long long fk_jprim1(long long tag, long long a);
static long long fk_jprim3(long long tag, long long a, long long b, long long c);
...
static void fk_jemit(long long i, int tail) {
 ...
 if (t == 1) { fk_jb1(0x48); fk_jb1(0xB8); fk_jb8(fk_node[i][1] << 1); return; }        /* int lit -> mov rax,imm64(tagged) */
 if (t == 24) { fk_jb1(0x48); fk_jb1(0xB8); fk_jb8(fk_node[i][1] << 1); return; }       /* STRING lit: tagged word = poolidx<<1 (known at lower-time, interned at parse) */
 ...
 if (t == 3 || t == 4 || t == 42 || t == 5 || t == 102) {
  /* float-aware arith/cmp. The #59 int-inline is WRONG for float operands, so we GUARD
     at runtime: if EITHER operand is a boxed float (tagged word <= fk_fbase-2, a huge
     negative), call the kind-correct carrier fk_jprim2 (bit-identical to fk_walk); else
     run the fast int-inline. This keeps fac/sum/fib native-fast (provably int at runtime)
     AND makes 0.5+0.25 correct — the float-correctness gate. */
  fk_jbin(i);                                          /* rax=left, rcx=right */
  /* threshold = fk_fbase - 2 (the float-band ceiling) */
  fk_jb1(0x49); fk_jb1(0xB9); fk_jb8(-9000000000000000000LL - 2); /* mov r9,fk_fbase-2 */
  fk_jb1(0x4C); fk_jb1(0x39); fk_jb1(0xC8);            /* cmp rax,r9 */
  fk_jb1(0x0F); fk_jb1(0x8E); long long jf1 = fk_jbp; fk_jb4(0); /* jle FLOAT (left is float) */
  fk_jb1(0x4C); fk_jb1(0x39); fk_jb1(0xC9);            /* cmp rcx,r9 */
  fk_jb1(0x0F); fk_jb1(0x8E); long long jf2 = fk_jbp; fk_jb4(0); /* jle FLOAT (right is float) */
  /* ── INT fast path (rax=left, rcx=right) ── */
  if (t == 3) { fk_jb1(0x48); fk_jb1(0x01); fk_jb1(0xC8); }                                /* add rax,rcx */
  else if (t == 4) { fk_jb1(0x48); fk_jb1(0x29); fk_jb1(0xC8); }                           /* sub rax,rcx */
  else if (t == 42) { fk_jb1(0x48); fk_jb1(0xD1); fk_jb1(0xF8); fk_jb1(0x48); fk_jb1(0xD1); fk_jb1(0xF9); fk_jb1(0x48); fk_jb1(0x0F); fk_jb1(0xAF); fk_jb1(0xC1); fk_jb1(0x48); fk_jb1(0xD1); fk_jb1(0xE0); } /* mul */
  else { /* le (5) / eq (102) */
   fk_jb1(0x48); fk_jb1(0xC7); fk_jb1(0xC2); fk_jb4(2);
   fk_jb1(0x49); fk_jb1(0xC7); fk_jb1(0xC0); fk_jb4(0);
   fk_jb1(0x48); fk_jb1(0x39); fk_jb1(0xC8);
   if (t == 5) { fk_jb1(0x4C); fk_jb1(0x0F); fk_jb1(0x4E); fk_jb1(0xC2); }
   else        { fk_jb1(0x4C); fk_jb1(0x0F); fk_jb1(0x44); fk_jb1(0xC2); }
   fk_jb1(0x4C); fk_jb1(0x89); fk_jb1(0xC0);
  }
  fk_jb1(0xE9); long long jdone = fk_jbp; fk_jb4(0);   /* jmp DONE */
  ...
  fk_jb1(0x51);                                        /* push rcx (arg2=right) */
  fk_jb1(0x50);                                        /* push rax (arg1=left) */
  fk_jb1(0x48); fk_jb1(0xC7); fk_jb1(0xC0); fk_jb4((int)t); /* mov rax,tag */
  fk_jb1(0x50);                                        /* push rax (arg0=tag) */
  fk_jcarrier((void *)fk_jprim2, 3);                   /* result in rax */
 ...
 if (t == 25 || t == 54 || t == 53) {                  /* str_len / float_to_int / str_to_float: 1-arg carrier */
  ...
  fk_jb1(0x48); fk_jb1(0xC7); fk_jb1(0xC0); fk_jb4((int)t); /* mov rax,tag */
  fk_jb1(0x50);                                        /* push rax (arg0=tag) */
  fk_jcarrier((void *)fk_jprim1, 2);                   /* result in rax */
  return;
 }
 if (t == 27 || t == 26 || t == 28) {                  /* str_concat / str_eq / str_byte_at: 2-arg carrier */
  fk_jemit(fk_node[i][1], 0); fk_jb1(0x50);            /* left -> push */
  fk_jemit(fk_node[i][2], 0); fk_jb1(0x50);            /* right -> push (top) */
  /* fk_jprim2(tag,a,b): arg0=tag@[rsp], arg1=a@[rsp+8], arg2=b@[rsp+16].
     Stack top->down is now right,left. Re-stage to tag,left,right. */
  fk_jb1(0x59);                                        /* pop rcx (right) */
  fk_jb1(0x58);                                        /* pop rax (left) */
  fk_jb1(0x51);                                        /* push rcx (arg2=right=b) */
  fk_jb1(0x50);                                        /* push rax (arg1=left=a) */
  fk_jb1(0x48); fk_jb1(0xC7); fk_jb1(0xC0); fk_jb4((int)t); /* mov rax,tag */
  fk_jb1(0x50);                                        /* push rax (arg0=tag) */
  fk_jcarrier((void *)fk_jprim2, 3);
  return;
 }
 if (t == 29) {                                        /* substring: 3-arg carrier fk_jprim3(tag,a,b,c) */
  fk_jemit(fk_node[i][1], 0); fk_jb1(0x50);            /* a -> push */
  fk_jemit(fk_node[i][2], 0); fk_jb1(0x50);            /* b -> push */
  fk_jemit(fk_node[i][3], 0); fk_jb1(0x50);            /* c -> push (top) */
  /* stack top->down: c,b,a. fk_jprim3 args: arg0=tag, arg1=a, arg2=b, arg3=c. Re-stage. */
  fk_jb1(0x41); fk_jb1(0x5A);                          /* pop r10 (c) */
  fk_jb1(0x59);                                        /* pop rcx (b) */
  fk_jb1(0x58);                                        /* pop rax (a) */
  fk_jb1(0x41); fk_jb1(0x52);                          /* push r10 (arg3=c) */
  fk_jb1(0x51);                                        /* push rcx (arg2=b) */
  fk_jb1(0x50);                                        /* push rax (arg1=a) */
  fk_jb1(0x48); fk_jb1(0xC7); fk_jb1(0xC0); fk_jb4(29);/* mov rax,29 */
  fk_jb1(0x50);                                        /* push rax (arg0=tag) */
  ...
  if (callee != fk_jit_self) {
   if (tail && argc <= 6 && fk_lower_tail_tramp) {
    /* ── TAIL inter-fn call → native trampoline form (the deep-mutual-recursion lever).
       Instead of a recursive fk_jcall (which grows the C stack), rewrite the new args INTO
       the current rbp frame IN PLACE (exactly like the tail SELF-call), record the next
       callee via fk_jtail_set, and RETURN the fk_tailcall sentinel. The enclosing fk_jtramp
       loops on that sentinel over the SAME frame → constant stack, native↔native end-to-end. */
    { long long k = 0; while (k < argc) { fk_jemit(an[k], 0); fk_jb1(0x50); k = k + 1; } } /* push arg0..argN-1 (top=argN-1) */
    { long long k = argc; while (k > 0) { k = k - 1; fk_jb1(0x58); /* pop rax (arg k) */ fk_jb1(0x48); fk_jb1(0x89); fk_jb1(0x45); fk_jb1((unsigned char)(k * 8)); } } /* mov [rbp+k*8],rax */
    fk_jb1(0x48); fk_jb1(0xC7); fk_jb1(0xC0); fk_jb4((int)callee); fk_jb1(0x50); /* mov rax,callee ; push (arg0 to carrier) */
    fk_jcarrier((void *)fk_jtail_set, 1);                 /* fk_jtail_set(callee); clobbers rax */
    fk_jb1(0x48); fk_jb1(0xB8); fk_jb8(fk_tailcall);      /* mov rax,fk_tailcall sentinel (the body's tail value) */
    return;
   }
   /* ── NON-TAIL inter-function call: emit a call to fk_jcall(callee, argc, argsptr). The
      carrier dispatches the callee (native via its own trampoline, or walker on deopt), so a
      JITed fn can call ANY other fn correctly — for ANY arity 0..6. We build the evaluated-args
      array on the machine stack (arg0 at the lowest address) and pass its pointer; fk_jcarrier's
      `and rsp,-16` only moves rsp DOWN, leaving this array intact above it. */
   long long k = argc;
...
static int fk_run_src(const char *path, long long arg) {
    ...
    and falls straight through to fk_walk. */
 {
  ...
  if (want) { fk_lower_tail_tramp = 1; }   /* --src JIT: tail inter-fn calls lower to the native trampoline */
  ...
  if (want && (rt == 12 || rt == 240 || rt == 241)) {
   ...
   if (callee >= 0 && callee < 4096) {
    ...
    if (n > 0) {
     ...
     long long ci = 0; while (ci < n) { img[ci] = fk_jb[ci]; ci = ci + 1; }
     fk_src_nat[callee] = img; fk_src_nat_len[callee] = n; fk_src_nat_frame[callee] = fk_jit_frame;
     fk_nat_tried[callee] = 1;   /* already crystallized; ensure_native must not re-lower */
     fk_njit = fk_njit + 1;
     if (getenv("FK_JIT_WITNESS")) { printf("[jit] fn%lld crystallized in-process: %lld bytes, njit=%lld (native dispatch)\n", callee, n, fk_njit); }
     /* install the root native ONCE and cache its exec ptr, so a sub-call that comes BACK to
        this fn (mutual recursion: ev->od->ev) reaches it by DIRECT native dispatch through
        fk_jcall, not the walker. The root call runs through the native TRAMPOLINE (fk_jtramp)
        so a top-level tail chain (ev/od) loops native↔native in constant stack — and the args
        frame fk_vs[0..) is rooted by fk_jtramp raising fk_vsp, so melt relocates any cons. */
     fk_nat_exec[callee] = fk_nat_install(img, n);
     long long rv;
     if (fk_nat_exec[callee] != 0 && ac == fk_fnar[callee]) {
      long long ai = 0; while (ai < ac) { if (ai < 65536) { fk_vs[ai] = aargs[ai]; } ai = ai + 1; }
      rv = fk_jtramp(callee, 0, ac);
     } else if (fk_nat_exec[callee] != 0) {
      long long fr = fk_jit_frame; if (fr < ac) { fr = ac; }
      long long ai = 0; while (ai < fr) { if (ai < 65536) { fk_vs[ai] = (ai < ac) ? aargs[ai] : 0; } ai = ai + 1; }
      long long save_vsp = fk_vsp; if (fr > fk_vsp && fr < 65536) { fk_vsp = fr; }
...
static int fk_run_feval(const char *path) {
 ...
 fk_fn_count = fk_defn_next;
  { char *je = getenv("FK_JIT"); fk_feval_jit_on = (je && je[0] && je[0] != 48) ? 1 : 0;   char *jh = getenv("FK_JIT_HOT"); if (jh && jh[0]) { long long h = atoi(jh); if (h > 0) { fk_feval_hot = h; } }   long long zi = 0; while (zi < 4096) { fk_fheat[zi] = 0; fk_nat_tried[zi] = 0; fk_nat_exec[zi] = 0; fk_src_nat[zi] = 0; zi = zi + 1; } }fk_vs[0] = 0; fk_vsp = 1;
 long long rv = fk_walk(fk_fn[0], 0);
 fk_pv(rv);   /* print the meta-eval result by value-kind (int / float / nothing) */
 return 0;
}
static int fk_run(int argc, char **argv) { if (argc < 2) { return 1; } if (argc >= 3 && argv[1][0] == 45 && argv[1][1] == 45 && argv[1][2] == 102 && argv[1][3] == 101) { return fk_run_feval(argv[2]); } if (argc >= 3 && argv[1][0] == 45 && argv[1][1] == 45) { return fk_run_src(argv[2], argc > 3 ? atoi(argv[3]) : 0); } int fd = open(argv[1], 0); if (fd < 0) { return 2; } long long got = read(fd, fk_buf, 1048575); if (got < 0) { return 3; } fk_buf[got] = 0; long long nf = fk_next(); fk_fn_count = nf; long long k = 0; while (k < nf) { fk_fn[k] = fk_next(); k = k + 1; } long long nr = fk_next(); fk_node_count = nr; long long r = 0; while (r < nr) { fk_node[r][0] = fk_next(); fk_node[r][1] = fk_next(); fk_node[r][2] = fk_next(); fk_node[r][3] = fk_next(); r = r + 1; } long long ns = fk_next(); fk_sinit(); long long si = 0; while (si < ns) { long long sl = fk_next(); if (fk_sp >= fk_scap_s) { fk_scap_s = fk_scap_s * 2; fk_so = realloc(fk_so, fk_scap_s * 8); fk_sl = realloc(fk_sl, fk_scap_s * 8); } while (fk_sbp + sl > fk_scap_b) { fk_scap_b = fk_scap_b * 2; fk_sb = realloc(fk_sb, fk_scap_b); } fk_so[fk_sp] = fk_sbp; fk_sl[fk_sp] = sl; long long bj = 0; while (bj < sl) { fk_sb[fk_sbp] = (char)fk_next(); fk_sbp = fk_sbp + 1; bj = bj + 1; } fk_sp = fk_sp + 1; si = si + 1; } long long a = 0; if (argc > 2) { a = atoi(argv[2]) << 1; } fk_src_len = 0; if (argc > 3) { int sfd = open(argv[3], 0); if (sfd >= 0) { long long sg = read(sfd, fk_src, 262143); if (sg >= 0) { fk_src_len = sg; fk_src[sg] = 0; } } } fk_vs[0] = a; fk_vsp = 1; if (argc > 4) { fk_hot = atoi(argv[4]); } if (argc > 5 && argv[5][0] == 106) { fk_nat_code[0] = fk_demo_inc; fk_nat_len[0] = 8; } long long rootv; fk_heat[0] = fk_heat[0] + 1; if (fk_hot > 0 && fk_heat[0] >= fk_hot && fk_nat_code[0] != 0) { fk_njit = fk_njit + 1; rootv = fk_native_call(fk_nat_code[0], fk_nat_len[0], fk_vs[0] >> 1) << 1; } else { rootv = fk_walk(fk_fn[0], 0); } fk_pv_root(fk_fn[0], rootv); long long t = 1; while (t <= 255) { fk_pr(fk_arms[t]); t = t + 1; } fk_pr(fk_njit); return 0; }
#if defined(_WIN32)
