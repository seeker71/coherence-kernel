/* GENERATED by flatten/gen-source-walker-table.fk from flt-ops
   (form-flatten.fk, the hand-maintained single source of truth). DO NOT HAND-EDIT.
   Adding a value op = a manifest row -> a flt-ops row -> regen -> an fkc-flat arm
   in form/form-stdlib/fkc-table-serialize.fk; NEVER a C edit. An op without its arm
   falls to the binary fallback there. Gate: gate/native-surface.bml */
struct fk_optab_row { const char *name; long long arity; long long tag; };
static const struct fk_optab_row fk_optab[] = {
  { "add", 2, 3 },
  { "sub", 2, 4 },
  { "le", 2, 5 },
  { "cons", 2, 19 },
  { "nth", 2, 23 },
  { "head", 1, 20 },
  { "tail", 1, 21 },
  { "len", 1, 22 },
  { "_plus", 2, 3 },
  { "str_len", 1, 25 },
  { "str_eq", 2, 26 },
  { "str_concat", 2, 27 },
  { "str_byte_at", 2, 28 },
  { "byte_to_str", 1, 33 },
  { "str_find", 3, 30 },
  { "input_byte", 1, 17 },
  { "read_line", 0, 114 },
  { "print_str", 1, 115 },
  { "isatty", 0, 116 },
  { "self_source", 0, 117 },
  { "kernel_stat", 1, 127 },
  { "fb_record", 3, 128 },
  { "framebuffer-events", 0, 129 },
  { "node_source", 1, 130 },
  { "node_at", 1, 147 },
  { "framebuffer-clear", 0, 131 },
  { "intern_trivial_int", 1, 43 },
  { "intern_trivial_bool", 1, 112 },
  { "intern_trivial_float", 1, 113 },
  { "make_nodeid", 4, 91 },
  { "bp", 1, 45 },
  { "intern_trivial_string", 1, 46 },
  { "intern_node", 2, 47 },
  { "node_children", 1, 48 },
  { "node_value", 1, 49 },
  { "node_category", 1, 92 },
  { "node_type", 1, 93 },
  { "node_inst", 1, 94 },
  { "node_pkg", 1, 95 },
  { "node_level", 1, 96 },
  { "node_eq", 2, 80 },
  { "value_eq", 2, 80 },
  { "math_floor", 1, 51 },
  { "floor", 1, 51 },
  { "round_ndigits", 2, 52 },
  { "str_to_float", 1, 53 },
  { "float_to_int", 1, 54 },
  { "math_sqrt", 1, 81 },
  { "trunc", 1, 82 },
  { "dot_product", 2, 84 },
  { "magnitude", 1, 85 },
  { "vector_cosine", 2, 86 },
  { "round", 1, 87 },
  { "ceil", 1, 88 },
  { "math_ceil", 1, 88 },
  { "math_exp", 1, 89 },
  { "math_log", 1, 90 },
  { "math_pow", 2, 195 },
  { "float_leaf", 2, 201 },
  { "host_dir_rmdir", 1, 55 },
  { "fs_rmdir", 1, 55 },
  { "host_dir_mkdir", 1, 56 },
  { "fs_mkdir", 1, 56 },
  { "host_path_exists", 1, 57 },
  { "fs_exists", 1, 57 },
  { "host_path_remove", 1, 58 },
  { "fs_remove", 1, 58 },
  { "host_path_rename", 2, 59 },
  { "fs_rename", 2, 59 },
  { "host_file_size", 1, 60 },
  { "file_size", 1, 60 },
  { "host_file_append_bytes", 2, 61 },
  { "file_append_bytes", 2, 61 },
  { "host_file_read_slice", 3, 62 },
  { "read_file_slice", 3, 62 },
  { "shm_offer", 2, 184 },
  { "shm_receive", 1, 185 },
  { "shm_write", 2, 186 },
  { "shm_read", 1, 187 },
  { "shm_seq", 1, 188 },
  { "shm_release", 1, 189 },
  { "terminal_cols", 0, 180 },
  { "terminal_lines", 0, 181 },
  { "host_monotonic_ms", 0, 182 },
  { "host_cpu_us", 0, 176 },
  { "host_gpu_utilization", 0, 174 },
  { "host_gpu_busy_us", 0, 175 },
  { "host_cpu_busy_us", 0, 173 },
  { "host_vm_stat", 0, 151 },
  { "host_load_avg", 0, 152 },
  { "host_disk_stat", 0, 153 },
  { "host_processes", 1, 154 },
  { "host_spawn", 1, 155 },
  { "host_wait", 1, 156 },
  { "host_kill", 1, 157 },
  { "host_nice", 1, 158 },
  { "host_capture", 1, 159 },
  { "host_pid", 0, 160 },
  { "host_cwd", 0, 29 },
  { "fs_truncate", 2, 143 },
  { "host_spawn_quiet", 1, 161 },
  { "kernel_live_pids", 0, 162 },
  { "kernel_live", 1, 163 },
  { "kernel_hot_rows", 1, 164 },
  { "metal_live", 0, 165 },
  { "gift_roster_register", 1, 166 },
  { "gift_roster_names", 0, 167 },
  { "cell_map", 1, 168 },
  { "cell_field", 3, 169 },
  { "cell_value", 2, 170 },
  { "cell_ref", 1, 171 },
  { "cell_unmap", 1, 172 },
  { "field_reset", 0, 196 },
  { "kernel_box_rows", 1, 191 },
  { "kernel_page_hot", 2, 192 },
  { "kernel_page_box", 2, 193 },
  { "host_sleep_ms", 1, 183 },
  { "kernel_hot", 1, 179 },
  { "node_gift_read", 1, 177 },
  { "node_gift_write", 2, 178 },
  { "kernel_ast", 2, 32 },
  { "host_file_read_text", 1, 63 },
  { "read_file", 1, 63 },
  { "host_temp_dir", 0, 101 },
  { "temp_dir", 0, 101 },
  { "host_file_write_text", 2, 104 },
  { "write_file", 2, 104 },
  { "write_file_text", 2, 104 },
  { "host_dir_list", 1, 132 },
  { "fs_list", 1, 132 },
  { "host_path_is_dir", 1, 200 },
  { "fs_is_dir", 1, 200 },
  { "host_source_inventory", 3, 202 },
  { "source_inventory", 3, 202 },
  { "metal_matvec_fixture", 0, 203 },
  { "metal_matvec_f32", 3, 204 },
  { "metal_pipeline", 2, 245 },
  { "metal_buf_alloc", 1, 246 },
  { "metal_buf_from_file", 3, 247 },
  { "metal_buf_write", 3, 248 },
  { "metal_enqueue", 3, 249 },
  { "metal_sync", 0, 250 },
  { "metal_buf_read", 3, 251 },
  { "metal_status", 0, 252 },
  { "metal_batch_concurrent", 0, 253 },
  { "metal_buf_free", 1, 254 },
  { "metal_submit", 0, 255 },
  { "metal_fence_wait", 1, 142 },
  { "metal_deadline", 1, 148 },
  { "sense_mic_count", 0, 205 },
  { "sense_cam_count", 0, 206 },
  { "sense_mic_name", 1, 207 },
  { "sense_cam_name", 1, 208 },
  { "sense_mic_health", 1, 209 },
  { "sense_cam_health", 1, 210 },
  { "sense_report", 0, 211 },
  { "sense_cam_grab", 1, 212 },
  { "jit_arm64_u32_leaf", 3, 215 },
  { "jit_leaf_inram", 2, 146 },
  { "sense_wifi_ssid", 0, 216 },
  { "sense_wifi_signal", 0, 217 },
  { "sense_bt_present", 0, 218 },
  { "sense_bt_count", 0, 219 },
  { "sense_power", 0, 220 },
  { "sense_mem", 0, 221 },
  { "sense_sensors", 0, 222 },
  { "sense_publish", 1, 223 },
  { "mesh_serve", 1, 224 },
  { "mesh_announce", 1, 225 },
  { "mesh_discover", 1, 226 },
  { "api_health", 0, 227 },
  { "mesh_register", 0, 228 },
  { "mesh_detect", 0, 229 },
  { "mesh_registry", 1, 230 },
  { "mesh_roster", 0, 231 },
  { "cuda_matvec", 0, 232 },
  { "cuda_matvec_f32", 2, 233 },
  { "sense_mic_capture", 1, 234 },
  { "sense_cam_luma", 1, 235 },
  { "sense_mic_stream_start", 0, 139 },
  { "sense_mic_stream_read", 2, 140 },
  { "sense_mic_stream_stop", 0, 141 },
  { "sense_audio_loopback", 1, 236 },
  { "sense_wav_loopback", 2, 237 },
  { "form_error", 1, 238 },
  { "print", -1, 239 },
  { "file_open", 1, 133 },
  { "file_read", 2, 134 },
  { "file_close", 1, 135 },
  { "host-exec", 2, 136 },
  { "http_get", 3, 105 },
  { "sock_request", 3, 118 },
  { "tls_request", 3, 119 },
  { "socket_listen", 1, 120 },
  { "socket_port", 1, 121 },
  { "socket_accept", 1, 122 },
  { "socket_connect", 2, 123 },
  { "socket_send", 2, 124 },
  { "socket_recv", 2, 125 },
  { "socket_close", 1, 126 },
  { "_get", 2, 106 },
  { "host_file_mtime", 1, 107 },
  { "file_mtime", 1, 107 },
  { "scan_run", 3, 108 },
  { "record_new", -1, 64 },
  { "record_get", 2, 65 },
  { "record_set", 3, 66 },
  { "method_define", 3, 197 },
  { "method_has", 2, 198 },
  { "method_invoke", -1, 199 },
  { "record_has", 2, 67 },
  { "record?", 1, 68 },
  { "record_keys", 1, 99 },
  { "record_blueprint", 1, 100 },
  { "now_unix_ms", 0, 15 },
  { "div", 2, 10 },
  { "mod", 2, 11 },
  { "band", 2, 34 },
  { "bor", 2, 35 },
  { "bxor", 2, 36 },
  { "shl_u32", 2, 37 },
  { "shr_u32", 2, 38 },
  { "rotr_u32", 2, 39 },
  { "add_u32", 2, 40 },
  { "bnot_u32", 1, 41 },
  { "mul", 2, 42 },
  { "empty", 0, 18 },
  { "list", -1, 19 },
  { "nothing", 0, 137 },
  { "nothing?", 1, 138 },

};
static const long long fk_optab_n = sizeof(fk_optab)/sizeof(fk_optab[0]);
struct fk_rwtab_row { const char *name; long long arity; long long nprog; long long prog[24]; };
static const struct fk_rwtab_row fk_rwtab[] = {
  { "gt", 2, 14, { 0,0,0,1,2,5,2,1,0,1,1,2,6,3,} },
  { "ge", 2, 7, { 0,1,0,0,2,5,2,} },
  { "lt", 2, 7, { 0,0,0,1,2,103,2,} },
  { "eq", 2, 7, { 0,0,0,1,2,102,2,} },
  { "and", 2, 16, { 0,0,0,1,1,1,1,0,2,6,3,1,0,2,6,3,} },
  { "or", 2, 16, { 0,0,1,1,0,1,1,1,1,0,2,6,3,2,6,3,} },
  { "not", 1, 9, { 0,0,1,0,1,1,2,6,3,} },
  { "ne", 2, 14, { 0,0,0,1,2,102,2,1,0,1,1,2,6,3,} },
  { "float_value", 1, 7, { 1,0,0,0,2,201,2,} },
  { "make_float32", 1, 7, { 1,1,0,0,2,201,2,} },
  { "make_float64", 1, 7, { 1,2,0,0,2,201,2,} },
  { "math_pi", 0, 7, { 1,3,1,0,2,201,2,} },
  { "value_kind", 1, 7, { 1,4,0,0,2,201,2,} },
  { "value-kind", 1, 7, { 1,4,0,0,2,201,2,} },
  { "recipe_to_bytes", 1, 7, { 1,5,0,0,2,201,2,} },
  { "bytes_to_recipe", 1, 7, { 1,6,0,0,2,201,2,} },
  { "read_form_binary", 1, 7, { 1,7,0,0,2,201,2,} },
  { "write_form_binary", 2, 12, { 1,8,0,0,0,1,2,19,2,2,201,2,} },
  { "substring", 3, 14, { 1,9,0,0,0,1,0,2,2,19,2,2,201,3,} },
  { "sense_speaker_count", 0, 7, { 1,10,1,0,2,201,2,} },
  { "sense_speaker_name", 1, 7, { 1,11,0,0,2,201,2,} },
  { "sense_speaker_health", 1, 7, { 1,12,0,0,2,201,2,} },
  { "sense_speaker_play", 1, 12, { 1,13,0,0,1,16000,2,19,2,2,201,2,} },
  { "sense_speaker_play_at", 2, 12, { 1,13,0,0,0,1,2,19,2,2,201,2,} },
  { "sense_speaker_stream_start", 0, 7, { 1,14,1,16000,2,201,2,} },
  { "sense_speaker_stream_start_at", 1, 7, { 1,14,0,0,2,201,2,} },
  { "sense_speaker_stream_write", 1, 7, { 1,15,0,0,2,201,2,} },
  { "sense_speaker_stream_stop", 0, 7, { 1,16,1,0,2,201,2,} },
  { "host_spawn_at", 2, 12, { 1,17,0,0,0,1,2,19,2,2,201,2,} },
  { "host_alive", 1, 7, { 1,18,0,0,2,201,2,} },
  { "fs_mkfifo", 1, 7, { 1,19,0,0,2,201,2,} },
  { "kernel_roster_adopt", 1, 7, { 1,20,0,0,2,201,2,} },
  { "kernel_roster_forget", 1, 7, { 1,21,0,0,2,201,2,} },
  { "kernel_page_bury", 1, 7, { 1,22,0,0,2,201,2,} },
  { "host_process", 1, 7, { 1,23,0,0,2,201,2,} },
  { "kernel_page_ended", 1, 7, { 1,24,0,0,2,201,2,} },
  { "value_str", 1, 7, { 1,25,0,0,2,201,2,} },
  { "int_to_str", 1, 7, { 1,26,0,0,2,201,2,} },
  { "abs", 1, 19, { 1,0,0,0,2,5,2,0,0,1,0,0,0,2,4,2,2,6,3,} },

};
static const long long fk_rwtab_n = sizeof(fk_rwtab)/sizeof(fk_rwtab[0]);
