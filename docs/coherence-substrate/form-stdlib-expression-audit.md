...
| --- | --- | --- | --- |
| JSON codec | `json-codec.bml` is the BML-authored codec declaration (`section [form.bml]` over `structured-codec.fk`); fkwu runs it directly and lowers it in memory when a `; preludes:` line names it. `json.fk` carries the plain-lane parser/emitter and the JSON cell categories, accessors, and constructors (`tests/json-band.fk` 1023, re-run 2026-09-04). `parse-json` still has plain-lane callers: `audit-evidence-cells.fk`, `json-lens-tissue.fk`, `mesh-sensings-route.fk`, `model-vitality.fk`, `nl-lexicon-grow.fk`, and seedbank. | `json-codec.fk` declares JSON as a structured codec; parse/emit enter through `JsonCodec`, and lower Form is generated or generic runtime execution. | Route the remaining `parse-json` callers through `JsonCodec`, then promote, generate, or release the plain parser/emitter explicitly. |
| HTTP stack | `http-parse.bml`, `http-render.bml`, `http-serve.fk`, `http-server.bml`, `http-socket.bml`, `http-adapter.bml`, and `kernel-http.fk` split protocol, routing, rendering, and serving across hand-authored functions. | HTTP is a protocol grammar plus route graph: request grammar, response grammar, route classes, observation cells, and socket/listener lifecycle as one front door. | Declare HTTP/1.1 parse/render as a protocol codec, then route serving consumes request/response cells only. |
...
| Field model | `field-model-form.fk` and `field-model-form-runtime.bml` contain many list/accessor and symbolic math helpers. | Field concepts are model cells with algebraic laws, observation, and reusable dimensional operators. | Split authored domain declarations from generic numeric/topology runtime support. |
| Algorithms and encodings | `base64.fk`, `hex.fk`, `url-encode.fk`, `rle.fk`, `uuid.fk`, `ulid.fk`, `sha256.fk`, `hmac-sha256.fk`, `crc32.fk`, `adler32.bml`, `dns.fk`. | Algorithm families are law declarations with reference vectors, possible native primitive realization, and generated runners. | Keep exact algorithms where needed, but wrap them in law/spec declarations and mark hot ones for native primitive/JIT treatment. |
