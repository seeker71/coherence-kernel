...
| --- | --- | --- | --- |
| JSON codec | `json-codec.fk` is the BML-authored codec declaration (`section [form.bml]` over `structured-codec.fk`), reachable through the BML lowering lane, not a plain `; preludes:` line. `json.fk` carries the plain-lane parser/emitter and the JSON cell categories, accessors, and constructors (`tests/json-band.fk` 1023, re-run 2026-09-04). `parse-json` still has plain-lane callers: `audit-evidence-cells.fk`, `json-lens-tissue.fk`, `mesh-sensings-route.fk`, `model-vitality.fk`, `nl-lexicon-grow.fk`, and seedbank. | `json-codec.fk` declares JSON as a structured codec; parse/emit enter through `JsonCodec`, and lower Form is generated or generic runtime execution. | Route the remaining `parse-json` callers through `JsonCodec`, then promote, generate, or release the plain parser/emitter explicitly. |
