...
`direct-source` becomes a compiler front door: it may read source and produce fresh
...
| R5 `direct-source` release | `direct-source` is an explicit compiler/admission spelling, not the only source invocation path | closed: plain `fkwu file.fk` uses the selector too |
