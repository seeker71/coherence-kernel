...
| Measure | Value |
| --- | ---: |
| repo visible `.fk` files | 3,211 |
| repo legacy anchored `; preludes:` declarations | 2,417 |
...
| R3a import declaration migration | `.fk` files use import declarations; `preludes:` remains only compatibility | runtime supports bare `import "path.fk"` and comment-safe `; import "path.fk"`; validator expands comment-safe imports recursively; 2,417 anchored legacy `; preludes:` declarations remain to migrate |
