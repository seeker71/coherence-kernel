...
the native `fkwu` kernel), and runs each as native tissue for the next
iteration. `fkwu` is the fourth arm `validate.sh` gates four-way and the runtime
whose heat lane crystallizes hot leaves to native and melts them on cool,
champion-challenger re-earned (`docs/form-native-jit-track.form`; manifest rows
`jit-lower fks 15`, `champion-challenger fks 127`). The SAME recipe IS the proof
(walked four-way) AND the native binary (crystallized on heat, or lowered through
the asm lever to a signed dylib) — no separate native impl, no clang dependency.
This doc is the living inventory + north-star alignment of that surface across
twelve families, and the open rework plan that keeps it supple. The floor source
for four-way coverage stays `form/fourth-arm-bands.txt` (907 rows on 2026-09-04);
this doc names purpose, grammar altitude, axiom relationships, and what is still
...
Paths, manifest rows, and the standing fkwu walls below were re-observed
2026-09-04; the per-family scores are this audit's own reading and are not
re-derived here.
...
10. host-os + IO + tool-channel — 62 (the host-OS kernel carriers cross four-way now that they verify through Form functions and observe recipes; carrier drift remains)
...
| file | grammar | aligned? | rework |
|---|---|---|---|
| form-cli-router | Form-recipe | yes | **make every ask route call fcr-route (sole router)** |
...
| file | grammar | aligned? | rework |
|---|---|---|---|
| host-kernel-cell | Form-recipe | yes | four-way (fks 255): carriers verify through Form functions, the body recipe is identity |
| kernel-satsang | Form-recipe | yes | four-way (fks 255): a part's verify is a Form function, images re-attach the waking kernel's verifiers; lift to BML tagged constructors |
| kernel-core-self/-image | mixed | yes | four-way (self 43, image fks 33): witness recipes observed, a missing witness voiced through organ-health; promote proof→band |
...
The manifest (form/fourth-arm-bands.txt, 907 rows on 2026-09-04) is the floor source and is largely accurate — but several concrete updates are owed so the body's self-attestation matches reality:

HOST-OS BANDS: the host-OS surface bands cross four-way — tool-channel-grammar (fks 255) and bml-capability-ledger (fks 255), both int/string/list ops the same family four-way tool-channel rides. Their kernel carriers cross too: kernel-satsang and host-kernel-cell verify each part through a Form function, and kernel-core-image and host-kernel-interface observe the witness recipes their images carry. No arm walks a recipe; the recipe walker natives are released from Go, Rust and TS, and fkwu reads and writes the .fkb image pair itself. kernel-image-proposal (fks 111111) and form-os-channel (fks 111111) cross four-way as Form recipes over their carriers. The http server value stack is registered four-way: http-render (63), http-parse (11), http-request (63), http-server (1023). ADD ROWS still owed: substrate-phase (its primitives already cross as cache-phase's prelude).
...
- **Why:** UNWIRED TRUST. form-cli-router carries the one fitness formula over sovereignty/trust/capability/confidence with weights-as-data — exactly right — but form-cli-loop's fcl-confidence is a second 0..100 confidence notion that never folds into the router's confidence axis. Trust/confidence is computed but not the actual gate at the live carrier.
- **Action:** Make every ask route call fcr-route so the four-axis formula is the only router; unify fcl-confidence with the router's confidence axis into one confidence primitive feeding both the gate and the fitness scalar.

### [FLOOR] The host-os bands cross four-way

- **Shape:** kernel-image-proposal (fks 111111), tool-channel-grammar (fks 255) and bml-capability-ledger (fks 255) cross as int/string/list recipes. kernel-satsang (fks 255) and host-kernel-cell (fks 255) verify each part through a zero-arg Form function; their .fkb images carry name, recipe and expected outside, and the waking kernel offers its verifiers by part name. kernel-core-image-compiler-proof (fks 33) and host-kernel-interface-proof (fks 37) observe the witness recipes their images carry, and each image compiler voices a missing witness through organ-health. No arm walks a recipe.
- **Remaining toward the native ideal:** host-kernel-interface-proof reads 37 of 40 on every arm — the BML exec-parse rows (parse ok, empty rest, self class) wait on the parser admitting host-kernel-interface.bml; the proof files use the *-proof.fk convention and become *-band.fk when those rows hold.
