...
- `direct-source` still has a C-seed parser/compiler path when artifacts are stale or
...
`direct-source` remains an explicit compiler/admission spelling. Plain `fkwu file.fk`
