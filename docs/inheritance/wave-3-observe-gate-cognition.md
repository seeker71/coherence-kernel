...
## How re-prove works on the source door (no toolchain)
...
BML core and was treated as **intrinsic to the source door**; concatenating it floored the run. As
of the 2026-06-30 direct-core refactor, `form/form-stdlib/core.fk` is parseable direct
Form and may be concatenated by new witnesses. A re-proven body's the source door witness equals
its **origin four-way witness** (`proven-bodies-from-old-repo.txt`).

### The `do/let`-block the source door floor — named exactly

the source door evaluates bare calls and nested `(add (if …) (if …))` correctly, but a band written as
...
| Body | Faculty | Composes (what it builds on) | origin | the source door | verdict |
|---|---|---|---|---|---|
...
| `nearest-shape` | `observe/` | core — the body's OWN classifier: content-addressing as recognition | `127` | `0` | HOME, dep-proven · standalone band the source door-pending (let floor) |
...
| `tool-channel` | `learn/` | core — native tool/channel catalog and planner | `255` | `0` | HOME, dep-proven · standalone band the source door-pending (let floor) |
| `choice-receipt` | `learn/` | core — shared branch/trust receipt compression | `4294967295` | `0` | HOME, dep-proven · standalone band the source door-pending (let floor) |
...
## Honest floor — 8 re-proven on the source door, 3 HOME-as-deps (named gap)
...
That is the named the source door gap, not a wrong answer: the bodies compute correctly (the consumer
...
the source door-proven. `nearest-shape`/`tool-channel`/`choice-receipt` are claimed exactly as
**home + transitively-proven + standalone-band-the source door-pending**.

## The the source door `do/let`-block gap

The `do/let`-block floor is the full shape: *any* `(do (let cN …) … )` band returns `0` on the source door,
...
nullary-defn band shape is the load-bearing one for the source door re-prove, and why most
