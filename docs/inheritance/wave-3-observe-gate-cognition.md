# Wave 3 — observe + gate + cognition faculties come home, re-proven on `fkwu`

The third wave of the homecoming (criterion #3 of [`INHERITANCE.md`](INHERITANCE.md)).
Like wave-2, every body here re-proves on the kernel's own **`fkwu` native lane** —
the c-bootstrapped source-runner, **no Go walker, no `retired sibling binary` flatten, no table load**.
...
## How re-prove works on `direct-source` (no toolchain)

`fkwu <file>` reads a single source file, parses it, walks `fk_fn[0]`, and prints the
root value. The method, byte for byte:

```
( cat <deps-first, NON-core> <body>.fk ; grep -v '^;' tests/<body>-band.fk ) > /tmp/b.fk
fkwu /tmp/b.fk    # -> the band's witness integer
```
...
BML core and was treated as **intrinsic to `direct-source`**; concatenating it floored the run. As
of the 2026-06-30 direct-core refactor, `form/form-stdlib/core.fk` is parseable direct
Form and may be concatenated by new witnesses. A re-proven body's `direct-source` witness equals
its **origin four-way witness** (`proven-bodies-from-old-repo.txt`).

### The `do/let`-block `direct-source` floor — named exactly

`direct-source` evaluates bare calls and nested `(add (if …) (if …))` correctly, but a band written as
...
`src` = the witness `fkwu` returns on **current-main `runtime/fkwu-uni.c`**, run from
the worktree's own ported copies; it must equal `origin` (the four-way witness from the old
repo) to read as re-proven.

| Body | Faculty | Composes (what it builds on) | origin | `direct-source` | verdict |
|---|---|---|---|---|---|
...
| `nearest-shape` | `observe/` | core — the body's OWN classifier: content-addressing as recognition | `127` | `0` | HOME, dep-proven · standalone band `direct-source`-pending (let floor) |
...
| `tool-channel` | `learn/` | core — native tool/channel catalog and planner | `255` | `0` | HOME, dep-proven · standalone band `direct-source`-pending (let floor) |
| `choice-receipt` | `learn/` | core — shared branch/trust receipt compression | `4294967295` | `0` | HOME, dep-proven · standalone band `direct-source`-pending (let floor) |
...
## Honest floor — 8 re-proven on `direct-source`, 3 HOME-as-deps (named gap)

**8 bodies reach their full origin witness on current-main `fkwu`** — proven native, no
...
That is the named `direct-source` gap, not a wrong answer: the bodies compute correctly (the consumer
...
`direct-source`-proven. `nearest-shape`/`tool-channel`/`choice-receipt` are claimed exactly as
**home + transitively-proven + standalone-band-`direct-source`-pending**.

## The `direct-source` `do/let`-block gap

The `do/let`-block floor is the full shape: *any* `(do (let cN …) … )` band returns `0` on `direct-source`,
...
nullary-defn band shape is the load-bearing one for `direct-source` re-prove, and why most
