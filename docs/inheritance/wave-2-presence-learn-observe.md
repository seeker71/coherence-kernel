# Wave 2 — presence + learn + observe faculties come home, re-proven on `fkwu`
...
body here **re-proves on the kernel's own `fkwu` native lane** — the
c-bootstrapped source-runner, **no Go walker, no `retired sibling binary` flatten, no table load**.
...
```
( cat presence/sibling-arrival.fk ; echo '(sibling-arrival-check)' ) | fkwu   # -> 11111
```
...
this wave, `core.fk` was the old BML core and was treated as **intrinsic to `direct-source`**;
...
`src` = the witness returned by `fkwu` on **current-main `runtime/fkwu-uni.c`**;
...
| `beings-channel` | `presence/` | core — one channel where every new being is present (the two rosters become one) | `11111` | `11101` | HOME, `direct-source`-partial · multi-arg gap |
...
| `learning-readout` | `learn/` | core, `temporal-sense` — one inspectable surface for "am I learning?" | `11111` | `10001` | HOME, `direct-source`-partial · multi-arg gap |
...
## Honest floor — 15 re-proven on `direct-source`, 2 HOME-but-`direct-source`-partial (named gap)

**15 bodies reach their full origin witness on current-main `fkwu`** — proven native,
no Go, no flatten.

**2 bodies are HOME (present, byte-exact, four-way-proven in origin) but `direct-source`-partial on
...
exactly — **multi-arg packed-call + nested-list access on `direct-source`** — and closes as that stone
...
as the arity lift lands. Recorded honestly as HOME-but-`direct-source`-PENDING-full per the ask.
...
The other 17 floored their `direct-source` check and are **NOT** ported — porting an unprovable body is
a claim without a receipt. They come home as the surface lands:

- **Float surface** (`direct-source` has no f64 lane — `(mul 2.5 2.0)` → `0`): `selective-ssm`,
...
Each is four-way-proven in origin; the float lane and the full string-pool surface on `direct-source`
