...
this wave, `core.fk` was the old BML core and was treated as **intrinsic to the source door**;
...
| `beings-channel` | `presence/` | core — one channel where every new being is present (the two rosters become one) | `11111` | `11101` | HOME, the source door-partial · multi-arg gap |
...
| `learning-readout` | `learn/` | core, `temporal-sense` — one inspectable surface for "am I learning?" | `11111` | `10001` | HOME, the source door-partial · multi-arg gap |
...
## Honest floor — 15 re-proven on the source door, 2 HOME-but-the source door-partial (named gap)

**15 bodies reach their full origin witness on current-main `fkwu`** — proven native,
no Go, no flatten.

**2 bodies are HOME (present, byte-exact, four-way-proven in origin) but the source door-partial on
...
exactly — **multi-arg packed-call + nested-list access on the source door** — and closes as that stone
...
as the arity lift lands. Recorded honestly as HOME-but-the source door-PENDING-full per the ask.
...
The other 17 floored their the source door check and are **NOT** ported — porting an unprovable body is
a claim without a receipt. They come home as the surface lands:

- **Float surface** (the source door has no f64 lane — `(mul 2.5 2.0)` → `0`): `selective-ssm`,
...
Each is four-way-proven in origin; the float lane and the full string-pool surface on the source door
